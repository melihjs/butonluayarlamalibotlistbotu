...
module.exports = {
  ...
  run: async (client, message, args) => {
    ...
    if(!message.member.roles.cache.has(yetkili)) return message.reply(red.setDescription(emoji.hayir+" Sen bunu yapamazsın!"));
    let botid = args[0];
    ...
    if(botid) {
      ...
      client.channels.cache.get(onayred).send(new MessageEmbed().setAuthor(message.author.username, message.author.displayAvatarURL({dynamic:true})).setFooter(client.user.username, client.user.displayAvatarURL({dynamic:true})).setColor('GREEN').setTimestamp().setThumbnail(message.guild.iconURL({dynamic:true})).setDescription(emoji.evet+" <@"+botid+"> adlı bot <@"+message.author.id+"> tarafından onaylandı!"));
      message.reply(new MessageEmbed().setAuthor(message.author.username, message.author.displayAvatarURL({dynamic:true})).setFooter(client.user.username, client.user.displayAvatarURL({dynamic:true})).setColor('GREEN').setTimestamp().setThumbnail(message.guild.iconURL({dynamic:true})).setDescription(emoji.evet+" <@"+botid+"> adlı bot onaylandı!"));
